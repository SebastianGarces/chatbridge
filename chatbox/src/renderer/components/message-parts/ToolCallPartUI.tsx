...
// Check if a tool result is an iframe render instruction from the ChatBridge backend
function isIframeRender(result: unknown): result is {
  type: 'iframe_render'
  appId: string
  appName: string
  appUrl: string
  ...
  return (
    ...
    result !== null &&
    (result as any).type === 'iframe_render' &&
    typeof (result as any).appUrl === 'string'
  )
}
...
export const ToolCallPartUI: FC<{ part: MessageToolCallPart }> = ({ part }) => {
  ...
  // Open iframe apps in the side panel from ChatBridge plugin system
  if (part.state === 'result' && isIframeRender(part.result)) {
    return <IframeRenderTrigger part={part} result={part.result} />
  }

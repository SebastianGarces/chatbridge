...
import AppPanel from '@/chatbridge/AppPanel'
import { useAppPanelStore } from '@/chatbridge/app-panel-store'
import AuthGate from '@/chatbridge/AuthGate'
...
function Root() {
  ...
  // Sync side panel with current session (only when session ID changes)
  ...
  useEffect(() => {
    ...
    if (sessionId !== lastSessionIdRef.current) {
      ...
      useAppPanelStore.getState().switchSession(sessionId)
    }
  }, [location.pathname])
